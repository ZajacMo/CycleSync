...
7. **围栏（Q3）**：以停车点密度生成候选围栏 → 覆盖选址（$\alpha=0.95,R=200$m）→ 贪心+剪枝+交换改进输出围栏方案。
...
再平衡量（投放为正、回收为负）：
$$
b_s=\mathrm{round}(\rho\cdot g_s),\quad \rho=1.1
$$
...
**（2）候选围栏点**  
对 $\mathcal{P}^{park}$ 运行 DBSCAN（haversine），半径 $r_f=50$m、`min_samples_f`=20，得到簇集合 $\mathcal{C}$；每簇中心作为候选围栏点 $j\in\mathcal{C}$，坐标 $(x_j,y_j)$。
...
**（4）候选规模控制**  
按 $w_i$ 取 Top-$M=300$ 个候选点，得到最终候选集合 $\mathcal{C}$ 与需求集合 $\mathcal{D}$（保持一致）。

### 3.2 覆盖约束选址模型（最小成本覆盖）

最大推行距离 $R=200$ m，覆盖率阈值 $\alpha=0.95$，单位成本 $f_j=1$。  
定义覆盖邻域：
$$
...
$$
$$
\sum_{i\in\mathcal{D}} w_i z_i \ge \alpha \sum_{i\in\mathcal{D}} w_i
$$

### 3.3 求解算法（贪心 + 剪枝 + 1-swap）
...
2. while $\sum_{i\in U}w_i < \alpha\sum_{i\in\mathcal{D}}w_i$：选择
$$
...
**Step C：剪枝（减少围栏数）**
按 $S$ 中围栏的“边际贡献”从小到大排序，依次尝试删除 $j\in S$：若删除后仍满足覆盖率约束，则永久删除。重复直至无可删点。

**Step D：1-swap 交换改进**
在剪枝结果上，尝试用 $S$ 外点替换 $S$ 内点：若替换后覆盖不降且后续可剪枝删除更多围栏，则接受；重复至收敛（最多 20 轮）。
...
**敏感性分析**
- $R\in\{150,200,300\}$ m；$\alpha\in\{0.90,0.95,0.98\}$；$r_f\in\{30,50,80\}$ m；$M\in\{200,300,500\}$。  
对每组参数输出 $|S|$ 与 $\mathrm{CR}$，并在地图上对比“未覆盖热点”。
